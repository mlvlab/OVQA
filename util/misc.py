...
def adjust_learning_rate(optimizer, curr_step: int, num_training_steps: int, args):
    ...
    if args.schedule == "linear_with_warmup":
        if curr_step < num_warmup_steps:
            ...
            gamma = max(0.0, float(num_training_steps - curr_step) / float(max(1, num_training_steps - num_warmup_steps)))
    else:  # constant LR
        gamma = 1
