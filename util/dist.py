...
def save_on_master(state_dict, checkpoint_path):
    """Utility function to save only from the main process"""
    model_without_deberta = {}
    for n, p in state_dict['model'].named_parameters():
        if p.requires_grad:
            model_without_deberta[n] = p
    state_dict['model'] = model_without_deberta
    if is_main_process():
