...
class GNNSoftVerbalizer(MessagePassing):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(in_channels, int):
            ...
            self.lin_dst = Linear(in_channels, heads * out_channels, bias=False, weight_initializer='glorot')
    ...
    def edge_update(self, x_j: Tensor, x_i: OptTensor, edge_attr: OptTensor, index: Tensor, ptr: OptTensor, size_i: Optional[int]) -> Tensor:
        # Given edge-level attention coefficients for source and target nodes,
        # we simply need to sum them up to "emulate" concatenation:
        
        _x_i, _x_j = self.lin_dst(x_i), self.lin_src(x_j)
        alpha = torch.bmm(_x_i, _x_j.transpose(1, 2)).squeeze(-1)
