...
import numpy as np
import torch
from torch import _softmax_backward_data, nn, optim
from torch.nn import CrossEntropyLoss, LayerNorm
import torch.nn.functional as F

from model.adapter import Adapter
from model.gnn import GNNSoftVerbalizer
...
class DebertaV2Encoder(nn.Module):
    ...
    def __init__(self, config, ds_factor_attn, ds_factor_ff, dropout):
        ...
        self.layer = nn.ModuleList([DebertaV2Layer(config, ds_factor_attn, ds_factor_ff, dropout) for _ in range(config.num_hidden_layers)])
...
class DebertaV2Embeddings(nn.Module):
    ...
    def __init__(self, config, features_dim):
        ...
        self.position_embeddings = nn.Embedding(config.max_position_embeddings, self.embedding_size)  # it is used for the decoder anyway
        ...
        # self.prompt_embedding = nn.Embedding(32, config.hidden_size)
    ...
    def forward(self, input_ids=None, token_type_ids=None, position_ids=None, mask=None, inputs_embeds=None, video=None):
        ...
        if inputs_embeds is None:
            ...
            if self.features_dim and video is not None:
                ...
                if self.prompt_embedding is not None:                    
                    inputs_embeds = torch.cat([video, inputs_embeds, self.prompt_embedding.weight[None, :, :].repeat(video.shape[0], 1, 1)], 1)
...
class DebertaV2Model(DebertaV2PreTrainedModel):
    def __init__(self, config, max_feats=10, features_dim=768, freeze_lm=False, ds_factor_attn=8, ds_factor_ff=8, ft_ln=False, dropout=0.1):
        super().__init__(config)

        self.embeddings = DebertaV2Embeddings(config, features_dim)
        self.encoder = DebertaV2Encoder(config, ds_factor_attn, ds_factor_ff, dropout)
        self.z_steps = 0
        self.config = config
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, position_ids=None, inputs_embeds=None, output_attentions=None, output_hidden_states=None,
                return_dict=None, video=None, video_mask=None):
        ...
        if self.features_dim and video is not None:
            ...
            # attention_mask = torch.cat([video_mask, attention_mask, torch.ones(video_mask.shape[0], 32).to(video_mask.device)], 1)
            input_shape = attention_mask.size()

        if token_type_ids is None:
            token_type_ids = torch.zeros(input_shape, dtype=torch.long, device=device)

        embedding_output = self.embeddings(input_ids=input_ids, token_type_ids=token_type_ids, position_ids=position_ids, mask=attention_mask, inputs_embeds=inputs_embeds, video=video)
        embedding_output, position_embeddings = embedding_output["embeddings"], embedding_output["position_embeddings"]
        ...
        return BaseModelOutput(last_hidden_state=sequence_output, hidden_states=encoder_outputs.hidden_states if output_hidden_states else None,
                               attentions=encoder_outputs.attentions, position_embeddings=position_embeddings, attention_mask=attention_mask)
...
class DebertaV2ForMaskedLM(DebertaV2PreTrainedModel):
    ...
    def __init__(self, config, max_feats=10, features_dim=768, freeze_lm=True, freeze_mlm=True, ds_factor_attn=8, ds_factor_ff=8, ft_ln=True,
                 dropout=0.1, n_ans=0, freeze_last=True, args=None):
        """
        ...
        self.deberta = DebertaV2Model(config, max_feats, features_dim, freeze_lm, ds_factor_attn, ds_factor_ff, ft_ln, dropout)
        self.lm_predictions = DebertaV2OnlyMLMHead(config, args)
        self.features_dim = features_dim
        if freeze_mlm:
        ...
        self.n_ans = n_ans
        if n_ans:
            self.answer_embeddings = nn.Embedding(n_ans, self.deberta.embeddings.embedding_size)
        ...
        self.lm_predictions.lm_head.gnn1.lin_src.weight.data = torch.eye(config.hidden_size)
        self.lm_predictions.lm_head.gnn2.lin_src.weight.data = torch.eye(config.hidden_size)
        self.lm_predictions.lm_head.gnn1.lin_dst.weight.data = torch.eye(config.hidden_size)
        self.lm_predictions.lm_head.gnn2.lin_dst.weight.data = torch.eye(config.hidden_size)
        self.lm_predictions.lm_head.gnn1.lin_src.weight.requires_grad_(True)
        self.lm_predictions.lm_head.gnn2.lin_src.weight.requires_grad_(True)
        self.lm_predictions.lm_head.gnn1.lin_dst.weight.requires_grad_(True)
        self.lm_predictions.lm_head.gnn2.lin_dst.weight.requires_grad_(True)
    ...
    def set_answer_embeddings(self, a2tok, freeze_last=True):
        ...
        if len(a2v) != self.n_ans:  # reinitialize the answer embeddings
            ...
            self.n_ans = len(a2v)
            self.answer_embeddings = nn.Embedding(self.n_ans, self.deberta.embeddings.embedding_size).to(self.device)
            self.answer_bias.requires_grad = False
            self.answer_bias.resize_(self.n_ans)
        self.answer_embeddings.weight.data = torch.div((a2v * (a2tok != pad_token_id).float()[:, :, None]).sum(1), sum_tokens.clamp(min=1))  # n_ans
        a2b = self.lm_predictions.lm_head.bias[a2tok]
        self.answer_bias.weight = torch.div((a2b * (a2tok != pad_token_id).float()).sum(1), sum_tokens.clamp(min=1))
        if freeze_last:
            self.answer_embeddings.requires_grad_(False)
            self.answer_bias.requires_grad_(False)
    
    def get_verbalized_embeddings(self, verbalized_id):
        verbalized_embeddings = self.deberta.embeddings.word_embeddings(verbalized_id)
        pad_token_id = getattr(self.config, "pad_token_id", 0)
        sum_tokens = (verbalized_id != pad_token_id).sum(1, keepdims=True)
        return verbalized_embeddings, sum_tokens
    ...
    def emd_context_layer(self, encoder_layers, z_states, attention_mask, encoder):
        ...
        if not self.config.position_biased_input:
            ...
            for layer in layers:
                output = layer(hidden_states, query_mask, return_att=False, query_states=query_states, relative_pos=None, rel_embeddings=rel_embeddings)
                query_states = output
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, position_ids=None, inputs_embeds=None, labels=None, output_attentions=None,
                return_dict=None, video=None, video_mask=None, mlm=False, eps=None, edge_index=None, vocab_embeddings=None):
        r"""
        ...
        outputs = self.deberta(input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids, position_ids=position_ids, inputs_embeds=inputs_embeds,
                               output_attentions=output_attentions, output_hidden_states=True, return_dict=return_dict, video=video, video_mask=video_mask)
        

        if labels is not None:
            if self.features_dim and video is not None:  # ignore the label predictions for visual tokens
                video_shape = video[:, :, 0].size()
                video_labels = torch.tensor([[-100] * video_shape[1]] * video_shape[0], dtype=torch.long, device=labels.device)
                labels = torch.cat([video_labels, labels], 1)

        # sequence_output = outputs[0]
        modified = self.emd_context_layer(encoder_layers=outputs["hidden_states"], 
                                          z_states=outputs["position_embeddings"].repeat(input_ids.shape[0] // len(outputs["position_embeddings"]), 1, 1), 
                                          attention_mask=outputs["attention_mask"], encoder=self.deberta.encoder)
        ...
        prediction_scores = self.lm_predictions(modified[-1], embeddings, edge_index, vocab_embeddings, eps, bias)
        masked_lm_loss = None
        if labels is not None:
            loss_fct = CrossEntropyLoss()  # -100 index = padding token
            masked_lm_loss = loss_fct(prediction_scores.view(-1, self.config.vocab_size), labels.view(-1))  # labels[labels > 0].view(-1)
...
class DebertaV2LMPredictionHead(nn.Module):
    def __init__(self, config, args):
        ...
        self.decoder = nn.Linear(config.hidden_size, config.vocab_size, bias=False)  # only for compatiblity
        ...
        self.gnn1 = GNNSoftVerbalizer(config.hidden_size, config.hidden_size)
        self.gnn2 = GNNSoftVerbalizer(config.hidden_size, config.hidden_size)
        self.args = args
        
    def forward(self, hidden_states, embedding_weight, edge_index, vocab_embeddings, eps, bias=None):
        hidden_states = self.dense(hidden_states)
        hidden_states = self.transform_act_fn(hidden_states)
        hidden_states = self.LayerNorm(hidden_states)

        if bias is not None:
            _embedding_weight = self.gnn1(vocab_embeddings, edge_index)
            _embedding_weight = self.gnn2(_embedding_weight, edge_index)[:embedding_weight.shape[0]]
            _embedding_weight = eps * embedding_weight + (1 - eps) * _embedding_weight

            logits = torch.matmul(hidden_states, _embedding_weight.t().to(hidden_states)) + bias
            

        else:
            logits = torch.matmul(hidden_states, embedding_weight.t().to(hidden_states)) + self.bias
...
class DebertaV2OnlyMLMHead(nn.Module):
    def __init__(self, config, args):
        super().__init__()
        self.lm_head = DebertaV2LMPredictionHead(config, args)

    def forward(self, sequence_output, embedding_weight, edge_index, vocab_embeddings, eps, bias=None):
        prediction_scores = self.lm_head(sequence_output, embedding_weight, edge_index, vocab_embeddings, eps, bias=bias)
